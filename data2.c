...
void DeleteFirst(PPNODE head)
{
	...
	//IF LL contain at least one node then store the address of 2nd node of 1st pointer through head 
 // And delete the frist node
 
 if(*head == NULL)
 {
	 return;
 }
 else //LL contains atleastone node
 {
	 temp = *head;
	 *head = temp->next;
	 free(temp);
